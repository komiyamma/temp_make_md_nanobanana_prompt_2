...
function Run-JulesForRange {
    ...
    $body = @{
        ...
        sourceContext       = @{
            source            = "sources/github/komiyamma/temp_make_md_nanobanana_prompt_2"
            githubRepoContext = @{ startingBranch = "main" }
        }
        requirePlanApproval = $false
        automationMode      = "AUTO_CREATE_PR"
        title               = "２番目。MDのnano banana 用の画像生成計画を立ててね。($targetRange)"
    ...
    if (-not $prUrl) {
        ...
        $prUrl = gh pr list --repo "komiyamma/temp_make_md_nanobanana_prompt_2" --limit 1 --json url --jq ".[0].url"
    }
...
if ($Loop) {
    ...
    for ($i = 1; $i -le 1000; $i += 6) {
        $r = "$i-$($i + 5)"
        while (-not (Run-JulesForRange -targetRange $r)) {
            Write-Warning "⚠️ 失敗したため、5秒後に再試行します: $r"
