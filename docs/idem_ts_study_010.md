...
![Idempotency-Key Flow](./picture/idem_ts_study_010_header_flow.png)
...
```mermaid
flowchart TD
    Start[リクエスト受信] --> KeyCheck{Keyはある?}
    KeyCheck -- No --> Error400[400 Bad Request]
    KeyCheck -- Yes --> ExistCheck{記録はある?}
    
    ExistCheck -- No --> Process[処理実行 & 結果保存]
    Process --> Success[レスポンス返却]
    
    ExistCheck -- Yes --> PayloadCheck{内容は同じ?}
    PayloadCheck -- No --> Error422[422 Payload Mismatch]
    PayloadCheck -- Yes --> StateCheck{状態は?}
    
    StateCheck -- processing --> Error409[409 Conflict]
    StateCheck -- done --> Replay[保存済み結果を返却]
```
