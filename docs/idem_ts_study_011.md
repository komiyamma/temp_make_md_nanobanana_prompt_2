...
![Concept](./picture/idem_ts_study_011_key_gen.png)
...
```mermaid
graph TD
    subgraph ClientGen[方式A: クライアント生成]
        C1[クライアント] -- "Key付きリクエスト" --> S1[サーバー]
        S1 -- "結果" --> C1
    end
    subgraph ServerGen[方式B: サーバー生成]
        C2[クライアント] -- "Key発行リクエスト" --> S2[サーバー]
        S2 -- "Key" --> C2
        C2 -- "そのKeyで実行" --> S2
        S2 -- "結果" --> C2
    end
