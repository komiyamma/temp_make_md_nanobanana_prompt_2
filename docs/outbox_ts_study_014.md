...
sequenceDiagram
    ...
    A->>DB: 1. 未送信ある？
    DB-->>A: Record X (pending)
    B->>DB: 2. 未送信ある？
    DB-->>B: Record X (pending)
    Note over A, B: 重複して拾っちゃう！😱
    A->>A: 送信する(1回目)
    B->>B: 送信する(2回目🔁)
```

![locking](./picture/outbox_ts_study_014_locking.png)
...
![Claim First Strategy](./picture/outbox_ts_study_014_claim_first.png)
...
![Status Flow with Processing](./picture/outbox_ts_study_014_status_flow.png)
...
![Skip Locked Visual](./picture/outbox_ts_study_014_skip_locked.png)
...
![Lease Expiry](./picture/outbox_ts_study_014_lease_expiry.png)
...
![Clock Skew Trap](./picture/outbox_ts_study_014_clock_skew.png)
