...
![Concept](./picture/idem_ts_study_018_error_buckets.png)

![Concept](./picture/idem_ts_study_018_error_buckets.png)

```mermaid
flowchart TD
    Error[エラー発生] --> Type{種類は?}
    Type -- 400/401/403/404 --> Fixed[恒久的: リトライNG ❌]
    Type -- 429/503 --> WaitRA[Retry-After待機後にリトライ 🔁]
    Type -- 500/502/504 --> Backoff[バックオフ後にリトライ 🔁]
    Type -- Timeout/Disconnect --> Idem{冪等キーある?}
    Idem -- No --> Risk[二重実行リスクあり ⚠️]
    Idem -- Yes --> Safe[安全にリトライ 🛡️]
```
...
「今たまたま無理だった」系。待ってもう一回で直ることが多い✨
例：ネットワークの瞬断📶、タイムアウト⏱️、サーバー過負荷🏋️、ゲートウェイエラー🌉
...
**タイムアウト** や **接続断** は特に危険！
「サーバー側では成功してたのに、返事だけ届かなかった」可能性があるから、**無邪気に再送すると二重実行**になりがち😱
ここは冪等性（Idempotency）と超相性がいいところだよ🔑✨
...
![Exponential Backoff + Jitter](./picture/idem_ts_study_018_backoff_viz.png)

連打すると、サーバーが余計に死ぬ（スローダウン→さらに失敗…）の無限ループになりがち😵‍💫
なので **指数バックオフ（exponential backoff）** が王道だよ📈✨ ([Amazon Web Services, Inc.][1])

![Exponential Backoff and Jitter](./picture/idem_ts_study_018_backoff_viz.png)

### ルール2：待ち時間に “ゆらぎ（jitter）” を入れる 🎲

みんなが同時に再試行すると「再試行の大渋滞」になるから、少しランダムにずらすのがコツ🎯 ([Amazon Web Services, Inc.][1])
...
`Retry-After` は **秒数** か **日時** で来るよ⏱️🗓️ ([rfc-editor.org][2])

### ルール5：HTTP的に “リトライしやすい操作” を知る 🔁🌐

HTTPでは **冪等なメソッド**（PUT/DELETE/安全なメソッドなど）は、通信失敗時に自動再試行しやすい性質として説明されてるよ🧠
「同じリクエストを繰り返しても、サーバー上の意図した効果が同じ」って定義される✨ ([rfc-editor.org][2])

### ルール6：POSTをリトライするなら “冪等キー” とセット 🔑💥

POSTは「作成＝増える」ので、無対策で再送すると二重作成しやすい😱
だから **Idempotency-Key を付ける**（＆サーバー側が同じキーなら同じ結果を返す）で安全にするよ🔐🔁
...
| ------- | -------------- | ------- | --------------------------------------------------------------------- |
| 429     | リクエスト多すぎ💦     | ✅       | `Retry-After` があれば待ってから再試行（なければバックオフ） ([MDNウェブドキュメント][3])             |
| 503     | 一時的に利用不可🛠️    | ✅       | できれば `Retry-After` に従う（サーバー側も一時的用途＆復旧見込みを載せるのが推奨） ([MDNウェブドキュメント][4]) |
...
| 404     | ないよ🔎          | ❌       | IDやURLを見直す                                                            |
| 409     | 競合⚔️           | △       | 状況次第：**最新状態を取得してから**やり直し、が多い（闇雲リトライはNG）                               |

> ポイント：**429/503は “待て” が明示される可能性が高い**から、まず `Retry-After` を探すのが勝ち筋だよ🏆 ([rfc-editor.org][2])
...
* ある → その秒数/日時まで待つ🫡⏳ ([rfc-editor.org][2])
* ない → 次へ

3. **ステータスが 429/503/5xx/408？**
...
（`Retry-After` が秒数or日時で来るのは仕様として明確だよ📌） ([rfc-editor.org][2])
...
* **`Retry-After` を無視してBANされる** 🥶（429のとき特に） ([MDNウェブドキュメント][3])
...
* 429/503は `Retry-After` を最優先で尊重🫡 ([rfc-editor.org][2])
* リトライは **バックオフ＋ジッター＋回数上限** が基本📈🎲 ([Amazon Web Services, Inc.][1])
