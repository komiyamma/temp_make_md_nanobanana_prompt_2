...
```mermaid
graph TD
    subgraph Envelope ["封筒 (Envelope) ✉️"]
        direction TB
        ID["id: ULID/UUID"]
        Source["source: /orders"]
        Type["type: order.confirmed.v1"]
        Time["time: 2026-02-03..."]
    end
    subgraph Payload ["便箋 (Payload) 📄"]
        direction TB
...
![envelope](./picture/outbox_ts_study_010_envelope.png)
