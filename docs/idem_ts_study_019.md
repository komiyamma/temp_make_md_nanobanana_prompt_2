...
![第19章失敗結果も保存する冪等__エラー保存戦略](./picture/idem_ts_study_019_ch19_idempotency_saving_failure_results_error_storage_strategy.png)
...
![2_失敗結果を保存するメリット](./picture/idem_ts_study_019_2_benefits_of_saving_failure_results.png)
...
![3_失敗結果を保存しないメリット](./picture/idem_ts_study_019_3_benefits_of_not_saving_failure_results.png)
...
![5_ざっくり早見表ミニ注文api想定](./picture/idem_ts_study_019_5_rough_reference_table_mini_order_api.png)
...
```mermaid
flowchart TD
    Error[エラー発生] --> Nature{エラーの性質は?}
    Nature -- 再実行しても無駄<br/>(400, 422, 残高不足) --> Save[結果を保存 📦]
    Nature -- 回復の可能性あり<br/>(503, 429, 一時DB障害) --> NoSave[保存しない or 短TTL ⏳]
    Nature -- 結果不明<br/>(Timeout) --> SaveFixed[副作用防止のため保存 🛡️]
    Save --> End[同じキーには同じエラーを返す]
    NoSave --> Retry[再送時に“初めから”やり直し]
```
...
![7_ミニ注文apiエラー保存つき_冪等ストア_実装方針a](./picture/idem_ts_study_019_7_mini_order_api_with_error_storage_idempotency_store_impl_plan_a.png)
...
![8_方針aの注意点ここだけは押さえてね](./picture/idem_ts_study_019_8_plan_a_key_points_to_remember.png)
...
![10_ai活用判断力を爆上げするプロンプト集](./picture/idem_ts_study_019_10_ai_usage_prompts_to_boost_judgment.png)
