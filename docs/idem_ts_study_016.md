...
![Concept](./picture/idem_ts_study_016_db_shield.png)
...
```mermaid
flowchart TD
    Start[リクエスト受信] --> Insert[INSERT try]
    Insert -- 成功 --> Process[初回: 処理開始 🚀]
    Insert -- 失敗: ユニーク違反 --> Select[既存レコード取得]
    Select --> Compare{Hashは一致?}
    Compare -- No --> Error409[409 Conflict]
    Compare -- Yes --> Replay[保存済み結果を返却 🔁]
```
